...
router_moderation = Router()
config = Config()

PENDING_POSTS_JSON = config.pending_posts_json
GROUPS_BY_GENRE = config.groups_by_genre
...
# ------------------ Команда /moderate -------------------
@router_moderation.message(Command("moderate"))
async def cmd_moderate(message: types.Message):
    """Запуск модерации"""
    bot = message.bot
    posts = load_json(PENDING_POSTS_JSON)

    chat = await bot.get_chat("@brook_get_s")  # либо используешь username канала
    print("Chat ID канала:", chat.id)
    ...
    # Передаём именно bot, а не message
